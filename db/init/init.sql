...
CREATE TABLE organization (
    ...
    CONSTRAINT chk_region CHECK (region IN ('SEOUL', 'BUSAN', 'DAEGU', 'INCHEON',
...
-- ENUM 값에 대한 CHECK 제약조건 추가 (NewsType 값에 따라 조정 필요)
ALTER TABLE news ADD CONSTRAINT chk_news_type CHECK (type IN ('NOTICE', 'WELFARE', 'RECRUIT', 'EVENT'));
...
CREATE TABLE member (
    ...
    CONSTRAINT chk_provider CHECK ( provider IN ('KAKAO')),
